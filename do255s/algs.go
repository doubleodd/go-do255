...
// This file implements high-level operations over do255s:
//
//   - Key pair generation
//   - Key exchange (ECDH)
...
// scalar for do255s. For efficiency reasons, it internally caches a
...
// non-zero scalar for do255s, then this function returns nil and an
// error.
func Do255sDecodePrivateKey(src []byte) (*Do255sPrivateKey, error) {
	if len(src) != 32 {
		return nil, errors.New("Invalid private key")
	}
	sk := new(Do255sPrivateKey)
...
// 32 bytes. If the provided slice does not have length exactly 32,
// or if what it contains is not the canonical encoding of a valid
// non-neutral do255s element, then this function returns nil and an
// error.
...
// Key pair generation with do255s: from a random source 'rand', a
...
// Key exchange with do255s: given our private key, and the public key
...
// bytes, or if it is not otherwise a valid do255s point encoding, then
...
func mkChallenge(dk *Do255sScalar, Renc []byte, epub []byte, hashOID []byte, data []byte) {
	sh := sha3.NewShake256()
	sh.Write([]byte("do255s-sign-e:"))
	sh.Write(Renc)
	sh.Write(epub)
	sh.Write(hashOID)
	sh.Write([]byte(":"))
...
// Schnorr signature with do255s. The data to sign ('data') may be either
...
func (sk *Do255sPrivateKey) signWithSeed(seed []byte, data []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	// Get hash OID.
	var hashOID []byte
...
func Do255sHashToCurve(data []byte, opts crypto.SignerOpts) (*Do255sPoint, error) {
	...
	sh := sha3.NewShake256()
	sh.Write([]byte("do255s-hash-to-curve:"))
	sh.Write(oid)
	sh.Write([]byte(":"))
	sh.Write(data)
	var bb [64]byte
	...
	P1.MapBytes(bb[:32])
	P2.MapBytes(bb[32:])
	return NewDo255sPoint().Add(&P1, &P2), nil
}
